...
const context = `
Eres Geminis, un chatbot especializado en ayudar con mi proyecto de tecnología Hardware.
...
relacionadas con lo que necesites saber de los componentes Hardware".
`;
...
app.post('/api/geminis', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.post("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyBu41Dv1wdEv_nCcVoc24X3Ptq-Rhs4VoY", {
